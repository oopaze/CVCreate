...
class CVWritter():
	...
	def escrever_formacao_academica(self):
		str_formacao = ''
		if self.__validate_conteiner(data['formacao'], 'FORMAÇÃO'):
	...
	def escrever_experiencia_profisional(self):
		str_experiencia = ''
		if self.__validate_conteiner(data['empregos'], 'EXPERIÊNCIA'):
	...
	def escrever_habilidades(self):
		str_habilidades = ''
		if self.__validate_conteiner(data['habilidades'], 'HABILIDADES'):
	...
	def escrever_projetos(self):
		str_projetos = ''
		if self.__validate_conteiner(data['projetos'], 'PROJETOS'):
